...
function addGamesToPage(games) {
    ...
    games.forEach( (game) => {
        ...
        <h2>${game.name}</h2>
        <img class="game-image" src="${game.img}" alt="${game.name} image failure" width="200" height="200"/>
...
const fundedGamesCount = GAMES_JSON.filter(game => {
    return game.pledged > game.goal;
}).length;
...
const summaryString = `
    ...
    We have successfully funded ${fundedGamesCount} game${fundedGamesCount !== 1 ? 's' : ''}, 
    and ${unfundedGamesCount} game${unfundedGamesCount !== 1 ? 's' : ''} currently remain unfunded. 
    We would love some help in funding these games!
`;
